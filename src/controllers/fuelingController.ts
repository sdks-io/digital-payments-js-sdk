...
} from '../errors/cancelFuelingErrorResponseError';
import {
  MppAccesTokenErrorResponseError,
} from '../errors/mppAccesTokenErrorResponseError';
import {
...
export class FuelingController extends BaseController {
  ...
  async mppToken(
    ...
    req.throwOn(401, MppAccesTokenErrorResponseError, 'Unauthorized. The request has not been applied because it lacks valid authentication credentials for the target resource.');
  ...
  async mppPrepareFueling(
    ...
    req.throwOn(400, ApiError, 'Error Occurred. Request did not include bearer token or token provided and is invalid.');
    req.throwOn(401, ApiError, 'Unauthorized');
    req.throwOn(403, ApiError, 'Forbidden. Requestor is not permitted to call the API');
    req.throwOn(404, ApiError, 'Not Found. Request received by the server but requested URL not found');
  ...
  async mppCancelFueling(
    ...
    req.throwOn(400, CancelFuelingErrorResponseError, 'Error Occurred. The server cannot or will not process the request due to an apparent client error (e.g., malformed request syntax, invalid request message). Please see below for information regarding structure of Response Body vs. all possible errors that could be returned.');
    req.throwOn(401, CancelFuelingErrorResponseError, 'Unauthorized. Request did not include bearer token or token provided and is invalid.');
    req.throwOn(403, ApiError, 'Forbidden. Requestor is not permitted to call the API.');
    req.throwOn(404, ApiError, 'Not Found. Request received by the server but requested URL not found');
    req.authenticate([{ oAuthTokenPost: true, mppToken: true }]);
